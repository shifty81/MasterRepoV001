#pragma once
/// @file BuiltinFont.h
/// @brief Embedded 8×14 monospace bitmap font covering ASCII 32–127.
///
/// Each glyph is stored as 14 bytes (one byte per scanline, MSB = leftmost
/// pixel).  The atlas covers 96 printable characters in the classic
/// "VGA 8×14" style — compact, crisp at 1× scale, and instantly legible
/// for editor / debug UI.
///
/// This data is in the public domain (based on the IBM VGA 8×14 codepage
/// 437 bitmap layout, which is not copyrightable as a typeface).
...
inline constexpr int kFontCellW = 8;
inline constexpr int kFontCellH = 14;

/// Number of printable ASCII characters (32 … 127).
inline constexpr int kFontGlyphCount = 96;

/// First ASCII code point represented in the atlas.
inline constexpr int kFontFirstChar = 32;

/// Raw 1-bit-per-pixel glyph data.
/// Index: (codepoint − 32) × 14 + row.
/// Each byte encodes 8 pixels; bit 7 = leftmost pixel.
// clang-format off
inline constexpr uint8_t kFontData[kFontGlyphCount * kFontCellH] = {
    // ' ' (32) — space
    ...
    // 'g' (103)
    0x00,0x00,0x00,0x00,0x00,0x76,0xCC,0xCC,0xCC,0xCC,0xCC,0x7C,0x0C,0x78,
    // 'h' (104)
    ...
    // 'p' (112)
    0x00,0x00,0x00,0x00,0x00,0xDC,0x66,0x66,0x66,0x66,0x66,0x7C,0x60,0xF0,
    // 'q' (113)
    0x00,0x00,0x00,0x00,0x00,0x76,0xCC,0xCC,0xCC,0xCC,0xCC,0x7C,0x0C,0x1E,
    // 'r' (114)
    ...
    // 'y' (121)
    0x00,0x00,0x00,0x00,0x00,0xC6,0xC6,0xC6,0xC6,0xC6,0xC6,0x7E,0x06,0xFC,
    // 'z' (122)
    ...
    // DEL (127) — rendered as a solid block
